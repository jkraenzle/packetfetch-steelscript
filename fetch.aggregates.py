...
def report_records_summarize (report_data, search_type, search_objects):
        ...
        for data_point in report_data:
                if (search_type == SEARCH_TYPE_IPADDRESS):
                        packet_count += data_point [1]
                        byte_count += data_point [2]
                else:
...
def report_columns_create (search_type, search_objects):
	...
	if (search_type == SEARCH_TYPE_IPADDRESS):
		columns = [Key ('tcp.ip'), Value ('sum_traffic.packets'), Value ('sum_traffic.total_bytes')]
	else:
		columns = [Key ('cli_tcp.ip'), Key ('srv_tcp.ip'), Value ('sum_traffic.packets'), Value ('sum_traffic.total_bytes')]

	return columns

def report_filter_str_create (search_type, search_objects):
	if (search_type == SEARCH_TYPE_IPADDRESS):
		report_filters = "tcp.ip == " + search_objects [0]
	else:
		report_filters = "(cli_tcp.ip == " + search_objects [0] + " and srv_tcp.ip == " + search_objects [1] \
			 + ") or (cli_tcp.ip == " + search_objects [1] + " and srv_tcp.ip == " + search_objects [0] + ")" \

	return report_filters

def find_top_talker (ar, columns, filter_str, timerange = None, topbycolumns=None):
	# Report
...
def main ():
	...
	for hostname in hostnamelist:
		...
		report_data = find_top_talker (ar, columns, filter_str, timerange)
